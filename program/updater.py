...
async def update_repo(_, message: Message):
    ...
    msg = await message.reply("🔄 `برمجه وكتابة بودآ المرازي♆...")
    update_avail = updater()
    if update_avail:
        await msg.edit("👤 يمكن شراء بوت بحقوقك كامل \n\n• او نسخه سورس بحقوقك بأرخص الأسعار حسابي الوحيد : Gaaaamd.")
        system("git pull -f && pip3 install -r requirements.txt")
        execle(sys.executable, sys.executable, "main.py", environ)
        return
        await msg.edit("My only **account on ** telegram [𝑩𝑶𝑫𝑨 𝑬𝑳𝑴𝑹𝑨𝒁𝑬 •](https://t.me/Gaaaamd)", disable_web_page_preview=True)
